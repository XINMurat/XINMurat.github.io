...
<section class="block">
  <div class="wrap">
    ...
    <ul class="plain">
      ...
      <li><a href="https://github.com/XINMurat/Mizan/blob/main/docs/refuted-patterns-excerpt.yaml"><code>refuted-patterns-excerpt.yaml</code></a> — the negative constraints, generated from the excerpt above by the export that already existed. It declares <code>partial: true</code>, because a list derived from an excerpt is a partial list of constraints; Kıyas's validator reads that flag and warns that a sweep against it may record "no match found" and may not record "clear". Two of the six patterns the batch was actually generated against are here. The other four are the ones about pricing and roadmap, and they stay private — so the claim that <em>those two discards were checked against six patterns</em> is still author-reported.</li>
...
<section class="block">
  <div class="wrap">
    ...
    <ul class="plain">
      ...
      <li><a href="https://github.com/XINMurat/Mizan/blob/main/docs/refuted-patterns-excerpt.yaml"><code>refuted-patterns-excerpt.yaml</code></a> — negatif kısıtlar; yukarıdaki alıntıdan, zaten var olan ihraç betiğiyle üretildi. <code>partial: true</code> ilan ediyor, çünkü bir alıntıdan türeyen liste kısmi bir kısıt listesidir; Kıyas'ın doğrulayıcısı bu bayrağı okuyup, buna karşı yapılan taramanın "eşleşme bulunamadı" yazabileceğini ama "temiz" yazamayacağını uyarıyor. Partinin gerçekten karşısında üretildiği altı örüntünün ikisi burada. Diğer dördü fiyat ve yol haritasıyla ilgili olanlar, ve özel kalıyor — dolayısıyla <em>o iki elemenin altı örüntüye karşı denetlendiği</em> iddiası hâlâ yazar-beyanı.</li>
